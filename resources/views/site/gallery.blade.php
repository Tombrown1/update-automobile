					...
					@foreach($gallerys as $gallery)
						<li>
							<div class="gallery-item">
								<img src="{{asset('storage/'.$gallery->file_path)}}" alt="Name of Photo">
								<div class="gallery-item-link">
									<span class="link-block">
										<a class="link link-more" href="{{route('gallery.view', ['slug' => $gallery->slug])}}"><em class="fa fa-link"></em></a>
										<a class="link link-popup image-lightbox" href="{{asset('storage/'.$gallery->file_path)}}" title="Photo Title"><em class="fa fa-arrows-alt"></em></a>
									</span>
								</div>
								<div class="gallery-item-caption dark">
									<p class="item-cat">Design</p>
									<h4 class="item-title">{{$gallery->name}}</h4>
								</div>
							</div>
						</li>
					@endforeach
